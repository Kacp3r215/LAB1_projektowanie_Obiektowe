...
public:
	...
	void GradientBackground(){
		Color topColor = { 108, 9, 133, 255 };
		Color bottomColor= BLACK;

		for (int y = 0; y < screenH; y++) {
			float ratio = (float)y / (float)screenH;
			Color color = {
...
public:
	...
	void Draw() const {
		if (type == WeaponType::BULLET) {
			...
			DrawRectangleRec(lr, GREEN);
		}
		else if (type == WeaponType::ROCKET) {
			Vector2 nose = { transform.position.x, transform.position.y - 15 };
			Vector2 base1 = { transform.position.x - 8, transform.position.y + 10 };
			Vector2 base2 = { transform.position.x + 8, transform.position.y + 10 };

			DrawTriangle(nose, base1, base2, ORANGE);
			DrawTriangleLines(nose, base1, base2, RED);

			if (rand() % 100 > 70) {
				Vector2 flame1 = { transform.position.x - 3, transform.position.y + 10 };
				Vector2 flame2 = { transform.position.x + 3, transform.position.y + 10 };
				Vector2 flameTip = { transform.position.x, transform.position.y + 20 };
				DrawTriangle(flame1, flame2, flameTip, YELLOW);
			}
	...
	float speed)
{
	...
	if (wt == WeaponType::LASER) {
		...
		return Projectile(pos, vel, 50, wt); //dodanie rakiety wysokie obrażenia 
	}
}
...
class Ship {
protected:
    float fireRateRocket = 8.f; // wolne strzały 
    float spacingRocket = 60.f;
    
public:
    Ship(int screenW, int screenH) {
        transform.position = { screenW * 0.5f, screenH * 0.5f };
        hp = 100;
        speed = 250.f;
        alive = true;
    ...
    float GetFireRate(WeaponType wt) const {
        switch (wt) {
            ...
            case WeaponType::ROCKET: return fireRateRocket;
            default: return fireRateLaser;
        }
    } 
    ...
    float GetSpacing(WeaponType wt) const {
        switch (wt) {
            ...
            case WeaponType::ROCKET: return spacingRocket;
            default: return spacingLaser;
        }
    }
...
public:
	...
	void Run() {
		...
		for (int i = 0; i < 500; i++) {
			gwiazdy.push_back({
				Utils::RandomFloat(0, Renderer::Instance().Width()),
				Utils::RandomFloat(0, Renderer::Instance().Height())
			});
		}
		...
		while (!WindowShouldClose()) {
			...
			// Shooting
			{
				if (player->IsAlive() && IsKeyDown(KEY_SPACE)) {
					...
					if (currentWeapon == WeaponType::ROCKET) {
						if (shotTimer >= interval * 3) {
							Vector2 p = player->GetPosition();
							p.y -= player->GetRadius();
							projectiles.push_back(MakeProjectile(currentWeapon, p, projSpeed * 0.7f));
			...
			// Render everything
			{
				...
				//gwiazdy 
				for (const auto& star : gwiazdy) {
					DrawPixelV(star, WHITE);
					
					if (GetRandomValue(0, 100) > 95) { // 5% szansy na większą gwiazdę
						DrawCircleV(star, 1.5f, WHITE);
					}
				}
				DrawText(TextFormat("HP: %d", player->GetHP()),
					10, 10, 20, GREEN);

				const char* weaponName = (currentWeapon == WeaponType::LASER) ? "LASER" : (currentWeapon == WeaponType::BULLET)? "BULLET":"ROCKET";
